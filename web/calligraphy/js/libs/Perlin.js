	...
	var Perlin = function() {
		...
		return {
			 noise: function(x,y,z) {
				...
				iYoffset = y;
				iZoffset = z;
			},generatePerlinMap: function(size, offset, zOffset, canvas) {
				var canvasPerlin;
				if(canvas == undefined) canvasPerlin = document.createElement("canvas");
				else canvasPerlin = canvas;
				
				canvasPerlin.width 	= size;
				canvasPerlin.height = size;
				var ctxPerlin = canvasPerlin.getContext('2d');	

				var data = ctxPerlin.getImageData(0, 0, size, size);
				var pixels = data.data;
				var i, j, k, index, r, g, b;
				for(j=0; j<size; j++) {
					for(i=0; i<size; i++) {
						r = this.noise(i*offset, j*offset, zOffset);
						g = this.noise(j*offset, i*offset, zOffset);
						b = this.noise(i*offset*2, j*offset*2, zOffset*2);
						// if(i == j == 0 ) console.debug( "Value : ", value );
						index = (i + j * size) * 4;
						pixels[index] = Math.floor(r * 255); 
						pixels[index+1] = Math.floor(g * 255); 
						pixels[index+2] = Math.floor(b * 255); 
						pixels[index+3] = 255; 
					}
				}

				ctxPerlin.putImageData(data, 0, 0);
				return canvasPerlin;
			}
		};
	}();
