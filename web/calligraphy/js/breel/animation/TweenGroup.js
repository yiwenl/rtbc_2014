	...
	if(!namespace.TweenGroup) {
		...
		p.start = function() {
			...
			for (var i=0, n=this.sequence.length; i<n; i+=3)
			{
				...
				setTimeout(function(t){t.start()}, (s[i+2]*1000)*this.scale, tween);
											
				maxLength = Math.max(maxLength, s[i+1]*this.scale + s[i+2]*this.scale);
			}
			
			// complete and update for entire group, using dummy tween
			TweenHelper.to({tick:0}, maxLength, 0, {tick:maxLength}).onComplete(this.onComplete).onUpdate(this.onUpdate).start();
		}
